...
func wnode(ctx *cli.Context) error {
	...
	if ctx.BoolT(WhisperInjectTestAccounts.Name) {
		if err = common.RestoreFile(filepath.Join(config.DataDir, "keystore"), "test-account3.pk"); err != nil {
			return err
		}
		if err = common.RestoreFile(filepath.Join(config.DataDir, "keystore"), "test-account4.pk"); err != nil {
			return err
		}
	}
