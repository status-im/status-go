...
func StartClient(seedPhrase string, path string) (*api.GethStatusBackend, error) {
	backend := api.NewGethStatusBackend()
        err := ImportAccount(seedPhrase, backend, path)
	if err != nil {
	...
	if err != nil {
        ...
        go retrieveMessagesLoop(wakuext.Messenger(), 300*time.Millisecond)
        return backend, nil
}
...
func ImportAccount(seedPhrase string, backend *api.GethStatusBackend, directory string) error {

        os.RemoveAll(directory)
	backend.UpdateRootDataDir(directory)
	manager := backend.AccountManager()
	if err := manager.InitKeystore(directory); err != nil {
		return err
	}
