...
func (b *GethStatusBackend) GetKeyUIDByMnemonic(mnemonic string) (string, error) {
	genAccount, err := b.accountManager.CreateFromMnemonicAndStoreAccount(mnemonic, "")
	if err != nil {
		return "", err
	}
...
func (b *GethStatusBackend) InitKeyStoreDirWithAccount(rootDataDir, keyUID string) (string, error) {
	...
	gethKeystore := keystore.NewKeyStore(keystoreAbsolutePath, keystore.LightScryptN, keystore.LightScryptP)
	adapter, err := geth.NewAdapter(keystoreAbsolutePath, gethKeystore)
	if err != nil {
		return "", err
	}
	b.accountManager.SetKeystore(adapter)
