...
func setupGethStatusBackend() (*backend.StatusBackend, func() error, func() error, func() error, error) {
	db, stop1, err := setupTestDB()
	if err != nil {
		return nil, nil, nil, nil, err
	}
	backend := backend.NewStatusBackend(testutils.MustCreateTestLogger())
	if err != nil {
		return nil, nil, nil, nil, err
	}
...
func TestBackendStartNodeConcurrently(t *testing.T) {
	...
	require.Contains(t, results, backend.ErrNodeRunning)
...
func TestBackendConnectionChangesToOffline(t *testing.T) {
	b := backend.NewStatusBackend(testutils.MustCreateTestLogger())
