...
func (s *MessengerCommunitiesTokenPermissionsSuite) TestImportDecryptedCodexArchiveMessages() {
	dataDir := s.T().TempDir()
	archiveDataDir := filepath.Join(dataDir, "codex", "archivedata")
	...
	// Ensure bob has archive (because they share same local directory)
	archiveIndex, err = s.bob.archiveManager.CodexLoadHistoryArchiveIndexFromFile(s.bob.identity, community.ID())
	s.Require().NoError(err)
	s.Require().Len(archiveIndex.Archives, 1)
...
func (s *MessengerCommunitiesTokenPermissionsSuite) TestFullCodexIntegration() {
	...
	indexCid, err := s.owner.archiveManager.GetHistoryArchiveIndexCid(community.ID())
	s.Require().NoError(err)
...
func (s *MessengerCommunitiesTokenPermissionsSuite) TestFullCodexIntegration2() {
	// skip for now - WIP
	s.T().Skip("WIP")
	...
	cancelChan := make(chan struct{})
	defer close(cancelChan)
	// taskInfo, err := s.bob.archiveManager.DownloadHistoryArchivesByIndexCid(community.ID(), indexCid, cancelChan)
	// we need to cancel the delayer, otherwise importHistoryArchives will wait for 5 minutes
	...
	state := &ReceivedMessageState{
		...
		Timesource:          s.bob.getTimesource(),
		CurrentMessageState: currentMessageState,
		ExistingMessagesMap: map[string]bool{},
		AllChats:            s.bob.allChats,
	}

	state.CurrentMessageState.PublicKey = &community.PrivateKey().PublicKey

	// message := &protobuf.CommunityMessageArchiveIndexCid{
	// 	Clock: 1000,
	// 	Cid:   indexCid,
	// }

	// err = s.bob.communitiesManager.SaveCommunitySettings(communities.CommunitySettings{
	// 	CommunityID:                  community.IDString(),
	// 	HistoryArchiveSupportEnabled: true,
	// })
	// s.Require().NoError(err)

	settings, err := s.bob.communitiesManager.GetCommunitySettingsByID(community.ID())
	s.Require().NoError(err)

	s.T().Logf("Bob community settings before download: %+v", settings)

	// s.bob.HandleCommunityMessageArchiveIndexCid(state, message, nil)
	s.bob.downloadAndImportCodexHistoryArchives(community.ID(), indexCid, cancelChan)
	s.Require().NoError(err)

	// // s.T().Logf("Codex download task info: %+v", taskInfo)
...
func (s *MessengerCommunitiesTokenPermissionsSuite) TestLoadingConfigFromDatabase() {
	...
	s.owner.EnableCommunityHistoryArchiveProtocol()
	s.bob.EnableCommunityHistoryArchiveProtocol()
	...
	indexCid, err = s.bob.archiveManager.GetHistoryArchiveIndexCid(community.ID())
	s.Require().NoError(err)
