...
func (r *storeNodeRequest) routine() {
	...
	ms := r.manager.messenger.getActiveMailserver(r.communityID)
	...
	_, err := r.manager.messenger.performMailserverRequest(ms, func(ms mailservers.Mailserver) (*MessengerResponse, error) {
		...
		return nil, r.manager.messenger.processMailserverBatchWithOptions(ms, batch, initialStoreNodeRequestPageSize, r.shouldFetchNextPage, true)
	})
