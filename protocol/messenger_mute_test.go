...
func (s *MessengerMuteSuite) TestSetMute() {
	...
	response, err := WaitOnMessengerResponse(
		...
		"public message not received",
	)

	// The chat unviewed messages count should not change
	s.Require().NoError(err)
	s.Require().Len(response.Chats, 1)
	s.Require().Equal(uint(0), response.Chats[0].UnviewedMessagesCount)

	// The message should be marked as seen if the chat is muted
	s.Require().NoError(err)
	s.Require().Len(response.Messages, 1)
	s.Require().True(response.Messages[0].Seen)
