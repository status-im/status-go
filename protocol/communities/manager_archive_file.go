...
// 	hasCid, err := m.codexClient.HasCid(lastSeenIndexCid)
// 	if err != nil {
// 		m.logger.Debug("[CODEX][createHistoryArchiveCodex] failed to verify Codex CID availability", zap.String("communityID", communityID.String()), zap.String("cid", lastSeenIndexCid), zap.Error(err))
// 		return codexArchiveIDs, err
// 	}

// 	if lastSeenIndexCid != "" && hasCid {
...
// 		m.logger.Debug("[CODEX][createHistoryArchiveCodex] updated last seen index cid in persistence", zap.String("cid", cid))
...
// 	index, err := m.CodexLoadHistoryArchiveIndex(m.identity, communityID, true)
// 	if err != nil {
// 		return nil, err
// 	}
...
// func (m *ArchiveFileManager) CodexLoadHistoryArchiveIndex(myKey *ecdsa.PrivateKey, communityID types.HexBytes, indexCid string, downloadTimeout time.Duration, isLocal bool) (*protobuf.CodexWakuMessageArchiveIndex, error) {
// 	codexWakuMessageArchiveIndexProto := &protobuf.CodexWakuMessageArchiveIndex{}
