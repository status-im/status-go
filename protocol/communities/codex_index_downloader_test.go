...
func (suite *CodexIndexDownloaderRealClientSuite) TestDownloadIndexFileFromLocalNode_Success() {
	// Arrange: Upload test data to Codex
	...
	cid, err := suite.client.Upload(bytes.NewReader(payload), "index-test.bin")
	require.NoError(suite.T(), err, "upload failed")
	suite.T().Logf("Upload successful, CID: %s", cid)
	suite.uploadedCIDs = append(suite.uploadedCIDs, cid) // Track for cleanup
...
func (suite *CodexIndexDownloaderRealClientSuite) TestDownloadIndexFileFromNetwork_Success() {
	...
	cid, err := suite.client.Upload(bytes.NewReader(payload), "network-index-test.bin")
	require.NoError(suite.T(), err, "upload failed")
	suite.T().Logf("Upload successful, CID: %s", cid)
	suite.uploadedCIDs = append(suite.uploadedCIDs, cid) // Track for cleanup
...
func (suite *CodexIndexDownloaderRealClientSuite) TestDownloadIndexFileFromLocalNode_LargeFile() {
	// Arrange: Upload a larger file (1MB)
	payload := make([]byte, 1024*1024)
	_, err := rand.Read(payload)
	require.NoError(suite.T(), err, "failed to generate random payload")
	suite.T().Logf("Generated large payload: %d bytes", len(payload))

	cid, err := suite.client.Upload(bytes.NewReader(payload), "large-index.bin")
	require.NoError(suite.T(), err, "upload failed")
	suite.T().Logf("Upload successful, CID: %s", cid)
	suite.uploadedCIDs = append(suite.uploadedCIDs, cid) // Track for cleanup
