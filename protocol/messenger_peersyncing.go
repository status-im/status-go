...
func (m *Messenger) handleDatasyncMetadata(response *messagingtypes.HandleMessageResponse) error {
...
func (m *Messenger) sendDatasyncOffersForChats() error {
	for _, chat := range m.Chats() {
		...
		_, err = m.messaging.SendPrivate(context.Background(), publicKey, &rawMessage)
		if err != nil {
			return err
		}
	}
...
func (m *Messenger) OnDatasyncOffer(response *messagingtypes.HandleMessageResponse) error {
	...
	_, err = m.messaging.SendPrivate(context.Background(), sender, &rawMessage)
	if err != nil {
		return err
	}
...
func (m *Messenger) OnDatasyncRequests(requester *ecdsa.PublicKey, messageIDs [][]byte) error {
	...
	for _, msg := range messages {
		...
		if lastRequested+30 < timeNow {
			...
			_, err = m.messaging.SendPrivate(context.Background(), requester, &rawMessage)
			if err != nil {
				return err
			}
		}
