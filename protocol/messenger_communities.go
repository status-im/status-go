...
func (m *Messenger) RequestImportDiscordCommunity(request *requests.ImportDiscordCommunity) {
	go func() {
		...
		for _, category := range exportData.Categories {
			...
			importProgress.UpdateTaskProgress(discord.CommunityCreationTask, 0.75+((float32(len(processedCategoriesIds))/float32(totalCategoriesCount))*0.25))
		...
		messageAssetsToDownload := map[string][]*protobuf.DiscordMessageAttachment{}
		...
		for _, channel := range exportData.ExportedData {
			...
			// know there was only a single such change
			for chatId, chat := range changes.ChatsAdded {
			...
			for _, discordMessage := range channel.Messages {
				...
				if err != nil {
					...
					importProgress.UpdateTaskProgress(discord.ImportMessagesTask, float32(len(processedMessages))/float32(totalMessageCount))
					continue
				}
				...
				if err != nil {
					m.logger.Error("failed if message author exists in database", zap.Error(err))
					if err != nil {
				...
				if err != nil {
					...
					// Q: Shall we add an import warning here?
					importProgress.AddTaskError(discord.ImportMessagesTask, discord.Warning(err.Error()))
					importProgress.UpdateTaskProgress(discord.ImportMessagesTask, float32(len(processedMessages))/float32(totalMessageCount))
					continue
				}
				...
				if len(discordMessage.Attachments) > 0 {
					messageAssetsToDownload[discordMessage.Id] = discordMessage.Attachments
				}
		...
		for authorId, avatarUrl := range authorAvatarUrls {
			...
			payload, imageType, imageBase64, err := discord.DownloadAndEncodeAvatarAsset(avatarUrl)
			if err != nil {
			...
			err = m.persistence.UpdateDiscordMessageAuthorImage(authorId, payload, *imageType, imageBase64)
			if err != nil {
		...
		for discordMessageId, attachments := range messageAssetsToDownload {
		  for _, attachment := range attachments {
        assetPayload, assetImageType, assetBase64, err := discord.DownloadAndEncodeImage(attachment.Url)
		    if err != nil {
		      errmsg := fmt.Sprintf("Couldn't download %s: %s", attachment.Url ,err.Error())
		      importProgress.AddTaskError(discord.DownloadAssetsTask, discord.Warning(errmsg))
		      m.publishImportProgress(importProgress)
		    }
        attachment.MessageId = discordMessageId
        attachment.Payload = assetPayload
        attachment.Type = assetImageType
        attachment.Base64 = assetBase64

        err = m.persistence.SaveDiscordMessageAttachment(attachment)
		    if err != nil {
		      errmsg := fmt.Sprintf("Couldn't save attachment %s: %s", attachment.Url ,err.Error())
