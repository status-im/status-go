...
func (s *EncryptionServiceTestSuite) TestHashRatchetCompatibility() {
	...
	_, err = s.alice.encryptor.persistence.DB.Exec("INSERT INTO hash_ratchet_encryption(group_id, key_id, key, hash_id) VALUES(?,?,?,?)", groupID, timestamp32, keyID1.Key, append(groupID, []byte("some-bytes")...))
	s.Require().NoError(err)

	// We migrate
	_, err = s.alice.encryptor.persistence.DB.Exec("UPDATE hash_ratchet_encryption SET key_timestamp = key_id")
	s.Require().NoError(err)
