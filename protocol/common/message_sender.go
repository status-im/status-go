...
func (s *MessageSender) EncodePrivate(
	ctx context.Context,
	recipient *ecdsa.PublicKey,
	rawMessage *RawMessage,
) (*types.NewMessage, error) {
	s.logger.Debug(
	...
	return s.encodePrivate(ctx, recipient, rawMessage)
}
...
func (s *MessageSender) EncodeCommunityMessage(
	ctx context.Context,
	rawMessage RawMessage,
) (*types.NewMessage, error) {
	s.logger.Debug(
	...
	return s.encodeCommunity(ctx, &rawMessage)
}

func (s *MessageSender) EncodeGroup(
	ctx context.Context,
	recipients []*ecdsa.PublicKey,
	rawMessage RawMessage,
) (*types.NewMessage, error) {
	return nil, errors.Wrap(nil, "not implemented")
}
...
func (s *MessageSender) encodeCommunity(
	ctx context.Context,
	rawMessage *RawMessage,
) (*types.NewMessage, error) {
	...
	if ShouldCommunityMessageBeEncrypted(rawMessage.MessageType) {
		...
		newMessage, err = s.encodeCommunityChatMessage(ctx, rawMessage, payload)
		if err != nil {
			return nil, err
		}
		...
		newMessage, err = s.encodeCommunityMessage(ctx, pubkey, payload, messageIDs)
		if err != nil {
			s.logger.Error("failed to send a community message", zap.Error(err))
			return nil, errors.Wrap(err, "failed to send a message spec")
		}

		s.logger.Debug("sent community message ", zap.String("messageID", messageID.String()))
	}
...
func (s *MessageSender) encodePrivate(
	...
	if s.featureFlags.Datasync && rawMessage.ResendAutomatically {
		...
		messageIDs := [][]byte{messageID}
		newMessage, err = s.encodePrivateRawMessage(ctx, rawMessage, recipient, wrappedMessage, messageIDs)
		if err != nil {
			s.logger.Error("failed to send a private message", zap.Error(err))
			return nil, errors.Wrap(err, "failed to send a message spec")
		}

		s.logger.Debug("sent private message skipEncryption", zap.String("messageID", messageID.String()))
		...
		messageIDs := [][]byte{messageID}
		newMessage, err = s.encodeMessageSpec(ctx, recipient, messageSpec, messageIDs)
		if err != nil {
			s.logger.Error("failed to send a private message", zap.Error(err))
			return nil, errors.Wrap(err, "failed to send a message spec")
		}

		s.logger.Debug("sent private message without datasync", zap.String("messageID", messageID.String()))
	}
...
func (s *MessageSender) encodeCommunityChatMessage(ctx context.Context, rawMessage *RawMessage, wrappedMessage []byte) (*types.NewMessage, error) {
	...
	err := s.transport.EncodePublic(ctx, newMessage, rawMessage.LocalChatID)
	if err != nil {
		return nil, err
	}
...
func (s *MessageSender) EncodePublic(
	ctx context.Context,
	chatName string,
	rawMessage RawMessage,
) (*types.NewMessage, error) {
	...
	err = s.transport.EncodePublic(ctx, newMessage, chatName)
	if err != nil {
		return nil, err
	}

	s.logger.Debug("sent public message", zap.String("messageID", messageID.String()))
...
func (s *MessageSender) encodePrivateRawMessage(ctx context.Context, rawMessage *RawMessage, publicKey *ecdsa.PublicKey, payload []byte, messageIDs [][]byte) (*types.NewMessage, error) {
	newMessage := &types.NewMessage{
		TTL:       whisperTTL,
		Payload:   payload,
	...
	if rawMessage.SendOnPersonalTopic {
		err = s.transport.EncodePrivateOnPersonalTopic(ctx, newMessage, publicKey)
	} else {
		err = s.transport.EncodePrivateWithPartitioned(ctx, newMessage, publicKey)
	}
	if err != nil {
		return nil, err
	}
...
// to a community
func (s *MessageSender) encodeCommunityMessage(ctx context.Context, publicKey *ecdsa.PublicKey, payload []byte, messageIDs [][]byte) (*types.NewMessage, error) {
	newMessage := &types.NewMessage{
		TTL:       whisperTTL,
		Payload:   payload,
		PowTarget: calculatePoW(payload),
...
func (s *MessageSender) encodeMessageSpec(ctx context.Context, publicKey *ecdsa.PublicKey, messageSpec *encryption.ProtocolMessageSpec, messageIDs [][]byte) (*types.NewMessage, error) {
	newMessage, err := MessageSpecToWhisper(messageSpec)
	if err != nil {
		return nil, err
	}
	...
	if messageSpec.AgreedSecret {
		...
		err = s.transport.EncodePrivateWithSharedSecret(ctx, newMessage, publicKey, messageSpec.SharedSecret.Key)
	} else {
