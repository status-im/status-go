...
func (s *MessengerStoreNodeRequestSuite) TestFetchingHistoryWhenOnline() {
	...
	s.Require().NoError(err)
	bobPeerID, err := bridge.GetGethWakuV2From(s.ownerWaku).PeerID()
	s.Require().NoError(err)

	s.logger.Debug("store node info", zap.Stringer("peerID", storePeerID))
	s.logger.Debug("owner node info", zap.Stringer("peerID", ownerPeerID))
	s.logger.Debug("bob node info", zap.Stringer("peerID", bobPeerID))
