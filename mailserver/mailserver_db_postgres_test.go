...
//
// Using Docker:
//   docker run --name mailserver-db -e POSTGRES_USER=test_user -e POSTGRES_PASSWORD=mysecretpassword -e POSTGRES_DB=test_db -d -p 5432:5432 postgres:9.6-alpine
//
...
func TestPostgresDB_BuildIteratorWithBloomFilter(t *testing.T) {
	...
	db, err := NewPostgresDB("postgres://test_user:mysecretpassword@127.0.0.1:5432/test_db?sslmode=disable")
