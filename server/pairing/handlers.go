...
func handlePairingReceive(ps *Server) http.HandlerFunc {
	signal.SendLocalPairingEvent(server.Event{Type: server.EventConnectionSuccess, Action: server.ActionPairingAccount})
	logger := ps.GetLogger()
	return func(w http.ResponseWriter, r *http.Request) {
		payload, err := io.ReadAll(r.Body)
		if err != nil {
			signal.SendLocalPairingEvent(server.Event{Type: server.EventTransferError, Error: err.Error(), Action: server.ActionPairingAccount})
			logger.Error("ioutil.ReadAll(r.Body)", zap.Error(err))
			return
		}
		signal.SendLocalPairingEvent(server.Event{Type: server.EventTransferSuccess, Action: server.ActionPairingAccount})

		err = ps.PayloadManager.Receive(payload)
		if err != nil {
			signal.SendLocalPairingEvent(server.Event{Type: server.EventProcessError, Error: err.Error(), Action: server.ActionPairingAccount})
			logger.Error("ps.PayloadManager.Receive(payload)", zap.Error(err))
			return
		}
		signal.SendLocalPairingEvent(server.Event{Type: server.EventProcessSuccess, Action: server.ActionPairingAccount})
	}
}

func handleParingSyncDeviceReceive(ps *Server) http.HandlerFunc {
	signal.SendLocalPairingEvent(server.Event{Type: server.EventConnectionSuccess, Action: server.ActionSyncDevice})
	logger := ps.GetLogger()
	return func(w http.ResponseWriter, r *http.Request) {
		payload, err := io.ReadAll(r.Body)
		if err != nil {
			signal.SendLocalPairingEvent(server.Event{Type: server.EventTransferError, Error: err.Error(), Action: server.ActionSyncDevice})
			logger.Error("io.ReadAll(r.Body)", zap.Error(err))
			return
		}
		signal.SendLocalPairingEvent(server.Event{Type: server.EventTransferSuccess, Action: server.ActionSyncDevice})

		err = ps.rawMessagePayloadManager.Receive(payload)
		if err != nil {
			signal.SendLocalPairingEvent(server.Event{Type: server.EventProcessError, Error: err.Error(), Action: server.ActionSyncDevice})
			logger.Error("ps.rawMessagePayloadManager.Receive(payload)", zap.Error(err))
			return
		}
		signal.SendLocalPairingEvent(server.Event{Type: server.EventProcessSuccess, Action: server.ActionSyncDevice})
	}
}
