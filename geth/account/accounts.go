...
func NewManager(provider ServiceProvider) *Manager {
	return &Manager{
		provider: provider,
	}
}
...
func (m *Manager) CreateChildAccount(parentAddress, password string) (address, pubKey string, err error) {
	keyStore, err := m.AccountKeyStore()
	if err != nil {
		return "", "", err
	}
...
// SetSelectedAccount returns currently selected account
func (m *Manager) SetSelectedAccount(a *SelectedExtKey) {
	m.selectedAccount = a
}
