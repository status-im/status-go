...
func (s *TopicPoolSuite) TestUsingCache() {
	...
	s.topicPool.ConfirmAdded(s.peer, p1.ID())
...
func (s *TopicPoolSuite) TestSyncSwitches() {
	...
	s.topicPool.ConfirmAdded(s.peer, p.ID())
	s.AssertConsumed(s.topicPool.period, s.topicPool.slowMode, time.Second)
	s.NotNil(s.topicPool.connectedPeers[p.ID()])
	s.topicPool.ConfirmDropped(s.peer, p.ID())
...
func (s *TopicPoolSuite) TestNewPeerSelectedOnDrop() {
	...
	s.topicPool.ConfirmAdded(s.peer, p1.ID())
	s.Contains(s.topicPool.connectedPeers, p1.ID())
	s.topicPool.ConfirmAdded(s.peer, p2.ID())
	s.Contains(s.topicPool.connectedPeers, p2.ID())
	s.topicPool.ConfirmAdded(s.peer, p3.ID())
	s.topicPool.ConfirmDropped(s.peer, p3.ID())
	s.Contains(s.topicPool.pendingPeers, p3.ID())
	s.Len(s.topicPool.pendingPeers, 1)
	s.Len(s.topicPool.discoveredPeersQueue, 1)
	// drop peer1
	s.True(s.topicPool.ConfirmDropped(s.peer, p1.ID()))
...
func (s *TopicPoolSuite) TestRequestedDoesntRemove() {
	...
	s.topicPool.ConfirmAdded(s.peer, p1.ID())
	s.topicPool.ConfirmAdded(s.peer, p2.ID())
	s.False(s.topicPool.connectedPeers[p1.ID()].dismissed)
	s.True(s.topicPool.connectedPeers[p2.ID()].dismissed)
	s.topicPool.ConfirmDropped(s.peer, p2.ID())
	s.Contains(s.topicPool.pendingPeers, p2.ID())
	s.NotContains(s.topicPool.connectedPeers, p2.ID())
	s.topicPool.ConfirmDropped(s.peer, p1.ID())
	s.NotContains(s.topicPool.pendingPeers, p1.ID())
	s.NotContains(s.topicPool.connectedPeers, p1.ID())
}
...
func (s *TopicPoolSuite) TestTheMostRecentPeerIsSelected() {
	...
	s.True(s.topicPool.pendingPeers[p2.ID()].discoveredTime < s.topicPool.pendingPeers[p3.ID()].discoveredTime)
	s.Equal(peer3.ID, s.topicPool.AddPeerFromTable(s.peer).ID)
}
...
func (s *TopicPoolSuite) TestMaxCachedPeers() {
	...
	s.False(s.topicPool.connectedPeers[p1.ID()].dismissed)
	s.True(s.topicPool.connectedPeers[p2.ID()].dismissed)
	s.True(s.topicPool.connectedPeers[p3.ID()].dismissed)
	...
	s.Contains(s.topicPool.connectedPeers, p1.ID())
	s.NotContains(s.topicPool.connectedPeers, p2.ID())
	s.NotContains(s.topicPool.connectedPeers, p3.ID())
	s.NotContains(s.topicPool.pendingPeers, p1.ID())
	s.Contains(s.topicPool.pendingPeers, p2.ID())
	s.Contains(s.topicPool.pendingPeers, p3.ID())
