...
func TestConnect(t *testing.T) {
	...
	connectingNodeConfig := Config{
		UseThrottledPublish: true,
		ClusterID:           16,
	}

	// start node that will initiate the connection
	connectingNodeWakuConfig := WakuConfig{
		EnableRelay:     true,
		LogLevel:        "DEBUG",
		Discv5Discovery: false,
		ClusterID:       16,
	...
	connectingNode, err := New(nil, "", &connectingNodeConfig, &connectingNodeWakuConfig, logger.Named("connectingNode"), nil, nil, nil, nil)
	require.NoError(t, err)
	require.NoError(t, connectingNode.Start())
	...
	// start node that will receive the connection
	receiverNodeWakuConfig := WakuConfig{
		EnableRelay:     true,
		LogLevel:        "DEBUG",
		Discv5Discovery: false,
	...
	// Check that both nodes start with no connected peers
	connectingPeerCount, err := connectingNode.PeerCount()
	require.NoError(t, err)
	require.True(t, connectingPeerCount == 0, "Connecting node should have no connected peers")
	...
	connectingPeerCount, err = connectingNode.PeerCount()
	require.NoError(t, err)
	require.True(t, connectingPeerCount == 1, "Connecting node should have 1 peer")
	...
	// Stop nodes
	require.NoError(t, connectingNode.Stop())
	require.NoError(t, receiverNode.Stop())
