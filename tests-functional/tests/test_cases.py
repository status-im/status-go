...
class MessengerTestCase(NetworkConditionTestCase):
    ...
    def setup_two_nodes(self, request):
        request.cls.sender = self.sender = self.initialize_backend(self.await_signals)
        request.cls.receiver = self.receiver = self.initialize_backend(self.await_signals)
