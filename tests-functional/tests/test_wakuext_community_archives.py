...
class TestCommunityArchives(MessengerSteps):
    @staticmethod
    def _wait_for_signal(backend, signal_type, timeout=20, event_pattern=None, start="beginning"):
        with backend.expect_signal(signal_type, timeout=timeout, pattern=event_pattern, start=start) as exp:
            pass
        return exp.result
    ...
    def test_community_archive_index_exists(self):
        ...
        self._wait_for_signal(self.member, SignalType.MESSAGES_NEW.value, timeout=10, event_pattern=chat_id)
        ...
        self._wait_for_signal(self.member, SignalType.MESSAGES_NEW.value, timeout=10, event_pattern=message_id)
        ...
        self._wait_for_signal(self.creator, SignalType.COMMUNITY_HISTORY_ARCHIVES_CREATED.value, timeout=message_archive_interval + 10)
        ...
        self._wait_for_signal(self.member, SignalType.COMMUNITY_ARCHIVE_INDEX_DOWNLOAD_COMPLETED.value, timeout=archive_timeout)
        ...
        self._wait_for_signal(self.member, SignalType.COMMUNITY_HISTORY_ARCHIVES_SEEDING.value, timeout=archive_timeout)
        ...
        self._wait_for_signal(self.another_member, SignalType.MESSAGES_NEW.value, timeout=10, event_pattern=chat_id)
        ...
        self._wait_for_signal(self.another_member, SignalType.COMMUNITY_ARCHIVE_INDEX_DOWNLOAD_COMPLETED.value, timeout=archive_timeout)
        ...
        self._wait_for_signal(self.another_member, SignalType.COMMUNITY_HISTORY_ARCHIVES_SEEDING.value, timeout=archive_timeout)
        ...
        self._wait_for_signal(self.another_member, SignalType.COMMUNITY_IMPORTING_HISTORY_ARCHIVE_MESSAGES_STARTED.value, timeout=archive_timeout)
        ...
        self._wait_for_signal(self.another_member, SignalType.COMMUNITY_IMPORTING_HISTORY_ARCHIVE_MESSAGES_FINISHED.value, timeout=archive_timeout)
    ...
    def test_archive_is_downloaded_after_logout_login(self):
        ...
        with self.member.expect_signal(SignalType.NODE_STOPPED.value, timeout=30, start="now"):
            self.member.logout()
        ...
        self.member.login(key_uid, password)
        self.member.wait_for_login()
        self.member.wait_for_wakuext_ready(timeout=30, start_messenger=False)
