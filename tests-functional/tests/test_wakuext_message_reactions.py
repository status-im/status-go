...
class TestMessageReactions(MessengerTestCase):
    ...
    def test_one_to_one_message_reactions(self):
        ...
        response = self.receiver.wakuext_service.rpc_request(method="sendEmojiReaction",
                                                             params=[receiver_chat_id, message_id, 1]).json()
        ...
        response = self.sender.wakuext_service.rpc_request(method="emojiReactionsByChatIDMessageID",
                                                           params=[sender_chat_id, message_id]).json()
        ...
        response = self.receiver.wakuext_service.rpc_request(method="sendEmojiReactionRetraction",
                                                             params=[emoji_id, ]).json()
        ...
        emoji_1_id = self.receiver.wakuext_service.rpc_request(
            method="sendEmojiReaction",
            params=[receiver_chat_id, message_1['id'], 2]
        ).json()['result']['emojiReactions'][0]['id']
        self.sender.find_signal_containing_pattern(
            SignalType.MESSAGES_NEW.value,
        ...
        emoji_2_id = self.sender.wakuext_service.rpc_request(
            method="sendEmojiReaction",
            params=[sender_chat_id, message_2['id'], 3]
        ).json()['result']['emojiReactions'][0]['id']
        ...
        time.sleep(10)
        response = self.sender.wakuext_service.rpc_request(method="emojiReactionsByChatID",
                                                           params=[sender_chat_id, None, 20]).json()
        self.sender.verify_json_schema(response, "wakuext_emojiReactionsByChatID")
        result = response["result"]
        assert len(result) == 2
        for item in result:
            assert all(
                (item['chatId'] == sender_chat_id, item['messageId'] == message_2['id'], item['emojiId'] == 3)
            ) or all(
