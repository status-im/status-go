...
if [[ -z "${FUNCTIONAL_TESTS_DOCKER_UID}" ]]; then
  FUNCTIONAL_TESTS_DOCKER_UID=$(id -u)
fi

if [[ -z "${FUNCTIONAL_TESTS_REPORT_CODECOV}" ]]; then
...
docker-compose ${all_compose_files} up -d --build --remove-orphans > "${root_path}/docker.log"
...
# Retrieve exit code
exit_code=$(docker inspect tests-functional-tests-rpc-1 -f '{{.State.ExitCode}}');
