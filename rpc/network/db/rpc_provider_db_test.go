...
func (s *RpcProviderPersistenceTestSuite) TestAddAndGetRpcProvider() {
	provider := testutil.CreateProvider(common.EthereumMainnet, "Provider1", params.UserProviderType, true, security.NewSensitiveString("https://provider1.example.com"))

	err := s.rpcPersistence.AddRpcProvider(provider)
	s.Require().NoError(err)

	// Verify the added provider
	providers, err := s.rpcPersistence.GetRpcProviders(common.EthereumMainnet)
	s.Require().NoError(err)
...
func (s *RpcProviderPersistenceTestSuite) TestGetRpcProvidersByType() {
	...
	// Verify by type
	userProviders, err := s.rpcPersistence.GetRpcProvidersByType(common.EthereumMainnet, params.UserProviderType)
	s.Require().NoError(err)
	testutil.CompareProvidersList(s.T(), []params.RpcProvider{providers[0], providers[2]}, userProviders)

	embeddedDirectProviders, err := s.rpcPersistence.GetRpcProvidersByType(common.EthereumMainnet, params.EmbeddedDirectProviderType)
	s.Require().NoError(err)
...
func (s *RpcProviderPersistenceTestSuite) TestDeleteRpcProviders() {
	...
	err = s.rpcPersistence.DeleteRpcProviders(common.EthereumMainnet)
	s.Require().NoError(err)
...
func (s *RpcProviderPersistenceTestSuite) TestUpdateRpcProvider() {
	...
	// Verify update
	updatedProviders, err := s.rpcPersistence.GetRpcProviders(common.EthereumMainnet)
	s.Require().NoError(err)
...
func (s *RpcProviderPersistenceTestSuite) TestSetRpcProviders() {
	initialProviders := []params.RpcProvider{
		testutil.CreateProvider(common.EthereumMainnet, "Provider1", params.UserProviderType, true, security.NewSensitiveString("https://provider1.example.com")),
		testutil.CreateProvider(common.EthereumMainnet, "Provider2", params.EmbeddedDirectProviderType, false, security.NewSensitiveString("https://provider2.example.com")),
	}
