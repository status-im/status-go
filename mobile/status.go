...
func initKeystore(keydir string) string {
	gethKeystore := keystore.NewKeyStore(keydir, keystore.LightScryptN, keystore.LightScryptP)
	adapter, err := geth.NewAdapter(keydir, gethKeystore)
	if err != nil {
		return makeJSONResponse(err)
	}
	statusBackend.AccountManager().SetKeystore(adapter)
	return makeJSONResponse(nil)
}
