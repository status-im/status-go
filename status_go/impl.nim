# Link flags for the artifacts `nim libstatus statusgo.nims` puts in the
# package's own build/bin, plus the syslibs and frameworks the Go runtime
# needs, so that a plain `import status_go` yields a linking program.
# Consumers that link their own flavour of the libraries (status-desktop links
# the shared one it builds itself) opt out with -d:statusGoNoAutoLink.
# The iOS and Android arms fall through to the macOS and Linux ones, which is
# wrong for both; mobile consumers always set -d:statusGoNoAutoLink.
when not defined(statusGoNoAutoLink):
  import std/os
  const statusGoLibDir = currentSourcePath().parentDir.parentDir / "build" / "bin"
  # libstatus by explicit path (not -l): build/bin also holds libstatus.dylib
  ...
  # libsds is linked SHARED. The static archive exports the whole Nim runtime,
  # which collides with the runtime of the program linking it (verified on
  # Linux; macOS is the exception). Hence also the rpath below.
  {.passL: "-L\"" & statusGoLibDir & "\" -lsds".}
  when defined(macosx):
    {.passL: "-Wl,-rpath,\"" & statusGoLibDir & "\"".}
    {.passL: "-framework CoreFoundation".}
    {.passL: "-framework Security".}
    {.passL: "-framework IOKit".}
    {.passL: "-lresolv".}
  elif defined(windows):
    # No rpath on Windows: libsds.dll must sit next to the executable or on PATH.
