...
func TestSecretResolver(t *testing.T) {
	...
	t.Run("resolved secret decrypts the keystore", func(t *testing.T) {
		...
		ok, err := accManager.VerifyAccountPassword(types2.HexToAddress(address), clientKEK)
		require.NoError(t, err)
		require.True(t, ok)
	})
	...
	t.Run("fallback to raw password after interrupted migration", func(t *testing.T) {
		...
		ok, err := accManager.VerifyAccountPassword(types2.HexToAddress(address), filePassword)
		require.NoError(t, err)
		require.True(t, ok)
	})
	...
	t.Run("wrong secret with wrong fallback still fails", func(t *testing.T) {
		...
		ok, err := accManager.VerifyAccountPassword(types2.HexToAddress(address), "wrong-password")
		require.Error(t, err)
		require.False(t, ok)
	})
}
