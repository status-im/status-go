...
var hopBridgeContractAddresses = map[uint64]map[common.Address]map[string]common.Address{ // map[chainID]map[tokenContractAddress]map[contractType]address
	walletCommon.EthereumMainnet: {
		...
		common.HexToAddress("0xae78736Cd615f374D3085123A210448E74Fc6393"): { // rETH
		...
		common.HexToAddress("0xB0c7a3Ba49C7a6EaBa6cD4a96C55a1391070Ac9A"): { // MAGIC
	...
	walletCommon.OptimismMainnet: {
		...
		common.HexToAddress("0x7F5c764cBc14f9669B88837ca1490cCa17c31607"): { // USDC.e
		...
		common.HexToAddress("0x8700dAec35aF8Ff88c16BdF0418774CB3D7599B4"): { // SNX
			L1CanonicalBridge:  common.HexToAddress("0x0000000000000000000000000000000000000000"),
		...
		common.HexToAddress("0x8c6f28f2F1A3C87F0f938b96d27520d9751ec8d9"): { // sUSD
			L1CanonicalBridge:  common.HexToAddress("0x0000000000000000000000000000000000000000"),
		...
		common.HexToAddress("0x9Bcef72be871e61ED4fBbc7630889beE758eb81D"): { // rETH
			L1CanonicalBridge:  common.HexToAddress("0x0000000000000000000000000000000000000000"),
	...
	walletCommon.ArbitrumMainnet: {
		...
		common.HexToAddress("0xEC70Dcb4A1EFa46b8F2D97C310C9c4790ba5ffA8"): { // rETH
			L1CanonicalBridge:  common.HexToAddress("0x0000000000000000000000000000000000000000"),
		...
		common.HexToAddress("0x539bdE0d7Dbd336b79148AA742883198BBF60342"): { // MAGIC
			L1CanonicalBridge:  common.HexToAddress("0x0000000000000000000000000000000000000000"),
	...
	walletCommon.BaseMainnet: {
		...
		common.HexToAddress("0xd9aAEc86B65D86f6A7B5B1b0c42FFA531710b6CA"): { // USDC.e
			L1CanonicalBridge:      common.HexToAddress("0x0000000000000000000000000000000000000000"),
	...
	walletCommon.EthereumSepolia: {
		...
		common.HexToAddress("0x95B01328BA6f4de261C4907fB35eE3c4968e9CEF"): { // USDC.e
	...
	walletCommon.OptimismSepolia: {
		common.HexToAddress("0x5fd84259d66Cd46123540766Be93DFE6D43130D7"): { // USDC
		...
		common.HexToAddress("0xB15312eA17d95375E64317C363A0e6304330D82e"): { // USDC.e
			L2CanonicalToken: common.HexToAddress("0xB15312eA17d95375E64317C363A0e6304330D82e"),
		},
	},
	walletCommon.ArbitrumSepolia: {
		common.HexToAddress("0x75faf114eafb1BDbe2F0316DF893fd58CE46AA4d"): { // USDC
...
func shouldUseCctpBridge(token *tokentypes.Token) bool {
	usdcContacts := []common.Address{
		...
		common.HexToAddress("0x5fd84259d66Cd46123540766Be93DFE6D43130D7"),
		common.HexToAddress("0x75faf114eafb1BDbe2F0316DF893fd58CE46AA4d"),
		common.HexToAddress("0x036CbD53842c5426634e7929541eC2318f3dCF7e"),
	}
	if slices.Contains(usdcContacts, token.Address) {
		return true
	}

	usdceContacts := []common.Address{
		...
		common.HexToAddress("0x95B01328BA6f4de261C4907fB35eE3c4968e9CEF"),
		common.HexToAddress("0xB15312eA17d95375E64317C363A0e6304330D82e"),
	}
	return slices.Contains(usdceContacts, token.Address) && isHTokenSend()
}

func IsHopToken(token *tokentypes.Token) bool {
	hopContacts := []common.Address{
		common.HexToAddress("0xc5102fE9359FD9a28f877a67E36B0F050d81a3CC"), // HOP token contract address is the same accross chains
	}
	return slices.Contains(hopContacts, token.Address)
}

func shouldUseAmm(token *tokentypes.Token) bool {
	return !IsHopToken(token)
}

func GetContractAddress(token *tokentypes.Token) (addr common.Address, contractType string, err error) {
	...
	if token.ChainID == walletCommon.EthereumMainnet ||
		...
		if addr, ok := hopBridgeContractAddresses[token.ChainID][token.Address][L1Bridge]; ok {
	...
	if shouldUseCctpBridge(token) {
		if addr, ok := hopBridgeContractAddresses[token.ChainID][token.Address][CctpL2Bridge]; ok {
			return addr, CctpL2Bridge, nil
		}
		return
	}

	if isHTokenSend() || !shouldUseAmm(token) {
		if addr, ok := hopBridgeContractAddresses[token.ChainID][token.Address][L2Bridge]; ok {
			return addr, L2Bridge, nil
		}
		return
	}

	if addr, ok := hopBridgeContractAddresses[token.ChainID][token.Address][L2AmmWrapper]; ok {
		return addr, L2AmmWrapper, nil
	}
	return
}

func GetTokenContractsAvailableOnChain(chainID uint64) (contractAddresses []common.Address) {
	mapByContractAddress, ok := hopBridgeContractAddresses[chainID]
	if !ok {
		return
	}
	return maps.Keys(mapByContractAddress)
}
