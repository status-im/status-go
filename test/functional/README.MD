...
The session-scoped `foundry_client` fixture waits up to nine minutes for Anvil
connectivity and the SNT, Communities, and ENS deployment address files. This
readiness check applies to local runs, Jenkins, and GitHub Actions. If Foundry
exits or deployment times out, setup reports its recent container logs. A failed
GitHub clone still requires fixing repository access; waiting cannot repair it.

### Nightly Logos Delivery compatibility tests

The **Test Logos Delivery Nightly** GitHub Actions workflow runs daily at 04:00 UTC
and supports **Run workflow** for on-demand runs. It always tests the latest
`develop` commit against `quay.io/wakuorg/nwaku-pr:nightly-logosdeliverynode` in a
local fleet, using the RPC suite with Logos Storage enabled. It does not run on
pull requests. The schedule becomes active once the workflow is on the default branch.
...
- Functional tests are implemented in `./test/functional/tests` based on [pytest](https://docs.pytest.org/en/8.2.x/)
...
and expects a Python virtual environment to be in `./test/functional/.venv`. 
...
This framework also hosts a suite of reliability tests. See more info [here](https://github.com/status-im/status-go/blob/develop/test/functional/tests/reliability/README.MD)
