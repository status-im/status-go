...
// MessagesSent passes messages confirmed as sent into the network.
func (m *MessengerSignalsHandler) MessagesSent(messageIDs [][]byte) {
	signal.SendMessagesSent(messageIDs)
}

// MessagesExpired passes messages that failed to be sent.
func (m *MessengerSignalsHandler) MessagesExpired(messageIDs [][]byte, err error) {
	signal.SendMessagesExpired(messageIDs, err)
}
