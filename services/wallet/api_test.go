...
func TestAPI_GetAddressDetails(t *testing.T) {
	...
	c, err := rpc.NewClient(nil, chainID, networks, appDB, walletDB, providerConfigs)
	require.NoError(t, err)
	...
	service := NewService(walletDB, accountsDb, appDB, c, accountFeed, nil, nil, nil, &params.NodeConfig{}, nil, nil, nil, nil, nil, "")
