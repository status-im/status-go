...
import (
	...
	"github.com/status-im/status-go/pkg/contracts/hop/l1Contracts/l1CctpImplementation"
	"github.com/status-im/status-go/pkg/contracts/hop/l1Contracts/l1Erc20Bridge"
	"github.com/status-im/status-go/pkg/contracts/hop/l2Contracts/l2ArbitrumBridge"
	"github.com/status-im/status-go/pkg/contracts/hop/l2Contracts/l2CctpImplementation"
...
func (h *HopBridgeProcessor) sendOrBuildV2(sendArgs *wallettypes.SendTxArgs, signerFn bind.SignerFn, lastUsedNonce int64) (tx *ethTypes.Transaction, err error) {
	...
	case hop.L2Bridge:
		tx, err = h.sendL2BridgeTx(contractAddress, ethClient, sendArgs.FromChainID, sendArgs.ToChainID, common.Address(*sendArgs.To), txOpts, bonderFee)
	default:
