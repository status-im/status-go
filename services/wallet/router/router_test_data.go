...
func getNormalTestParamsList() []normalTestParams {
	return []normalTestParams{
		{
			name: "ETH transfer - No Specific FromChain",
			input: &requests.RouteInputParams{
				TestnetMode: false,
			},
			expectedError: requests.ErrFromChainNotSupported,
		},
		{
			name: "ETH transfer - No Specific ToChain",
			input: &requests.RouteInputParams{
				TestnetMode: false,
				FromChainID: walletCommon.EthereumMainnet,
			},
			expectedError: requests.ErrToChainNotSupported,
		},
		{
		...
		{
			name: "ETH transfer - Some AmountIn",
			input: &requests.RouteInputParams{
				TestnetMode: false,
				Uuid:        uuid.NewString(),
				SendType:    sendtype.Transfer,
		...
		{
			name: "ETH transfer - All AmountIn",
			input: &requests.RouteInputParams{
				TestnetMode: false,
				Uuid:        uuid.NewString(),
				SendType:    sendtype.Transfer,
			...
			expectedCandidates: routes.Route{
				{
					...
					ToChain:          &mainnet,
					ApprovalRequired: false,
					AmountOut:        (*hexutil.Big)(big.NewInt(1999999000000000000)),
				},
			},
		},
		{
			name: "ETH transfer - Different Chains",
			input: &requests.RouteInputParams{
				TestnetMode: false,
				Uuid:        uuid.NewString(),
				SendType:    sendtype.Transfer,
			...
			expectedError: ErrPathNotAvaliableForProvidedParameters,
		},
		{
			name: "ERC20 transfer - Insufficient Funds",
			input: &requests.RouteInputParams{
				TestnetMode: false,
				Uuid:        uuid.NewString(),
				SendType:    sendtype.Transfer,
