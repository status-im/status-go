...
//
//	mockgen -package=mock_paraswap -source=types.go -destination=mock/types.go
//
...
// BuildTransactionWithRetry mocks base method.
func (m *MockClientInterface) BuildTransactionWithRetry(ctx context.Context, srcTokenAddress common.Address, srcTokenDecimals uint, srcAmountWei *big.Int, destTokenAddress common.Address, destTokenDecimals uint, destAmountWei *big.Int, slippageBasisPoints uint, addressFrom, addressTo common.Address, priceRoute json.RawMessage, side paraswap.SwapSide) (paraswap.Transaction, *paraswap.Route, error) {
	m.ctrl.T.Helper()
	ret := m.ctrl.Call(m, "BuildTransactionWithRetry", ctx, srcTokenAddress, srcTokenDecimals, srcAmountWei, destTokenAddress, destTokenDecimals, destAmountWei, slippageBasisPoints, addressFrom, addressTo, priceRoute, side)
	ret0, _ := ret[0].(paraswap.Transaction)
	ret1, _ := ret[1].(*paraswap.Route)
	ret2, _ := ret[2].(error)
	return ret0, ret1, ret2
}

// BuildTransactionWithRetry indicates an expected call of BuildTransactionWithRetry.
func (mr *MockClientInterfaceMockRecorder) BuildTransactionWithRetry(ctx, srcTokenAddress, srcTokenDecimals, srcAmountWei, destTokenAddress, destTokenDecimals, destAmountWei, slippageBasisPoints, addressFrom, addressTo, priceRoute, side any) *gomock.Call {
	mr.mock.ctrl.T.Helper()
	return mr.mock.ctrl.RecordCallWithMethodType(mr.mock, "BuildTransactionWithRetry", reflect.TypeOf((*MockClientInterface)(nil).BuildTransactionWithRetry), ctx, srcTokenAddress, srcTokenDecimals, srcAmountWei, destTokenAddress, destTokenDecimals, destAmountWei, slippageBasisPoints, addressFrom, addressTo, priceRoute, side)
}
