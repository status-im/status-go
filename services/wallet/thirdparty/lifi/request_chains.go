...
// ChainInfo carries the per-chain contract addresses LI.FI publishes on /chains. Read from
// the API rather than hardcoded: every chain has its own Permit2Proxy deployment and the
// Permit2 singleton isn't canonical everywhere (Abstract and zkSync run a different one).
// Signing against the wrong verifying contract produces a signature the proxy can't use.
...
// SupportsPermit2 reports whether the chain exposes both contracts needed to route a
// swap through the Permit2Proxy.
func (c ChainInfo) SupportsPermit2() bool {
	return c.Permit2 != (common.Address{}) && c.Permit2Proxy != (common.Address{})
}
...
// FetchChains reads the full EVM chain list from LI.FI. Chains are returned as-is;
// callers are expected to check SupportsPermit2 before relying on the addresses.
...
// parseAddress turns an optional address field into a common.Address, yielding the
// zero address for the empty/absent case so callers can test it with SupportsPermit2.
