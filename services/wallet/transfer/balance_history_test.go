...
func TestComputeBlockInfoForTimestamp(t *testing.T) {
	bh, cleanDB := setupTestBalanceHistoryDB(t)
	defer cleanDB()

	dataSource := newTestSource(t, 5 /*years*/)
	calibData, err := bh.getOrFetchCalibrationData(context.Background(), dataSource)
	require.NoError(t, err)
...
func TestSnapTimestamp(t *testing.T) {
	...
	expectedTimestamp = time.Date(2011, 11 /*M*/, 1 /*d*/, 0 /*H*/, 0 /*m*/, 0 /*s*/, 0 /*ms*/, time.UTC).Unix()
	require.Equal(t, expectedTimestamp, snappedTimestamp)

	testDate = time.Date(2003, 3 /*M*/, 2 /*d*/, 23 /*H*/, 34 /*m*/, 56 /*s*/, 567 /*ms*/, time.UTC)
	snappedTimestamp = snapTimestamp(testDate.Unix(), BalanceHistoryAllTime)
	expectedTimestamp = time.Date(2003, 3 /*M*/, 1 /*d*/, 0 /*H*/, 0 /*m*/, 0 /*s*/, 0 /*ms*/, time.UTC).Unix()
	require.Equal(t, expectedTimestamp, snappedTimestamp)
}
...
func TestGetBalanceHistoryForBlocksSourceFetchAllTime(t *testing.T) {
	...
	require.Greater(t, timeRange, uint64(expectedDuration-timeErrorAllowed.Seconds()))
	require.Less(t, timeRange, uint64(expectedDuration+timeErrorAllowed.Seconds()))
