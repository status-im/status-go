...
var timeIntervalToSnapUnit = map[BalanceHistoryTimeInterval]snapUnit{
	...
	BalanceHistoryAllTime: snapUnitYear,
}
...
const calibrationCurrency = ""
const calibrationBitset = 1
...
func (bh *BalanceHistory) getOrFetchCalibrationData(ctx context.Context, chainClient BlockInfoSource) ([]*blockInfo, error) {
	...
	if len(dbData) > 0 {
		...
		nextBlockNo.Add(lastBlockInfo.block, big.NewInt(int64(float64(secondsInYear)/lastAvgBlockTime)))
	...
	for currentTime-lastBlockInfo.timestamp > secondsInYear {
		...
		nextBlockNo.Add(nextBlockNo, big.NewInt(int64(float64(secondsInYear)/lastAvgBlockTime)))
	}
...
func computeBlockInfoForTimestamp(timestamp int64, currentCalibIdx int, calib []*blockInfo, timeInterval BalanceHistoryTimeInterval) (block *big.Int, blockTimestamp int64, calibIdx int) {
	if currentCalibIdx == 0 {
		currentCalibIdx = 1
	}
	calibIdx = currentCalibIdx
	for i := currentCalibIdx; i < len(calib) && calib[i].timestamp < timestamp; i++ {
		calibIdx = i
	}
	_, _, currentAvgBlockTime := getStepInfo(calib[calibIdx-1], calib[calibIdx], timeInterval)
	timeToLastKnownBlock := timestamp - calib[calibIdx-1].timestamp
	blocksCount := int64(math.Floor(float64(timeToLastKnownBlock) / currentAvgBlockTime))
...
func (bh *BalanceHistory) getBalanceHistoryFromBlocksSource(ctx context.Context, chainClient BlockInfoSource, address common.Address, timeInterval BalanceHistoryTimeInterval) ([]*BalanceState, error) {
	...
	if timeInterval != BalanceHistoryAllTime {
		...
		currentBlockNumber = calib[0].block
