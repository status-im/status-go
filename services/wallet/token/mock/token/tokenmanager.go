...
//
//	mockgen -source=token.go -destination=mock/token/tokenmanager.go
//
...
func (m *MockManagerInterface) GetTokensByChainIDs(chainIDs []uint64) ([]*tokentypes.Token, error) {
	m.ctrl.T.Helper()
	ret := m.ctrl.Call(m, "GetTokensByChainIDs", chainIDs)
	ret0, _ := ret[0].([]*tokentypes.Token)
	ret1, _ := ret[1].(error)
	return ret0, ret1
}
...
func (m *MockManagerInterface) LookupToken(chainID *uint64, tokenSymbol string) (*tokentypes.Token, bool) {
	m.ctrl.T.Helper()
...
func (m *MockManagerInterface) LookupTokenIdentity(chainID uint64, address common.Address, native bool) *tokentypes.Token {
	m.ctrl.T.Helper()
