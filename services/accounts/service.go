...
func (s *Service) Start() error {
	gethKeystore := keystore.NewKeyStore(s.config.KeyStoreDir, keystore.LightScryptN, keystore.LightScryptP)
	adapter, err := geth.NewAdapter(s.config.KeyStoreDir, gethKeystore)
	if err != nil {
		return err
	}
	s.manager.SetKeystore(adapter)
	return nil
}
