...
func TestUsedConnectionPersisted(t *testing.T) {
	...
	monitor := NewLastUsedConnectionMonitor(store, cache, whisper)
	monitor.Start()

	// Send a confirmation that we received history from one of the peers.
	select {
	case whisper.input <- whisperv6.EnvelopeEvent{
		Event: whisperv6.EventMailServerRequestCompleted, Peer: nodes[0].ID()}:
	...
	select {
	case whisper.input <- whisperv6.EnvelopeEvent{
		Event: whisperv6.EventMailServerRequestCompleted, Peer: nodes[1].ID()}:
	case <-time.After(time.Second):
		require.FailNow(t, "can't send a 'completed' event")
	}
