...
func (s *EncryptionServiceMultiDeviceSuite) TestProcessPublicBundle() {
	...
	s.Require().NoError(err)
	s.Require().Equal([2]string{alice2Identity, "alice2"}, response[0])
	...
	s.Require().Equal([2]string{alice1Identity, "alice1"}, response[0])
	s.Require().Equal([2]string{alice2Identity, "alice2"}, response[1])
}
