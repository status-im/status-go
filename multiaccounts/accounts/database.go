...
func (db *Database) SaveSetting(setting string, value interface{}) error {
	...
	case "bookmarks":
		...
		update, err = db.db.Prepare("UPDATE settings SET bookmarks = ? WHERE synthetic_id = 'id'")
	case "chaos-mode?":
		_, ok := value.(bool)
		if !ok {
