...
func createTestMessage(pubsubTopic string, contentTopic types.ContentTopic, payload []byte) *types.ReceivedMessage {
...
func TestClient_PushReceivedMessages(t *testing.T) {
	...
	filter := *types.NewChatFilter(
		&types.ChatFilterConfig{
			PubsubTopic:  "test-pubsub",
			ContentTopic: types.StringToContentTopic("test-content"),
			ChatID:       "test-chat",
		},
	)

	shhMessage := createTestMessage("test-pubsub", types.StringToContentTopic("test-content"), []byte("test-payload"))
